...
```{r RankLevelLinearmodel}
m_rank_relevel.lm <- lm(score ~ rank, data = rank_relevel)
summary(m_rank_relevel.lm)
```

[Note: I've ended up with only one rank at this stage, which prevents a comparison of groups...]
...
[Note: I know I needed to do the above command, so I feel good about that. However, the next step requires either filter or rename]

```{r NewVarandRename}
evals <- rename(evals, c(teaching="no", tenure track= "yes", "tenured"= "yes"))
```
...
[Note: As abobe, because I've ended up with only one rank at this stage, this prevents a comparison of groups...]
